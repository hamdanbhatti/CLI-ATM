...
while (loginAttempts < maxLoginAttempts) {
    ...
    if (userPin === 8877) {
        // Correct PIN entered
        console.log(`${chalk.yellow("...... W E L C O M E .....")}\n
      This Code Written By ${chalk.green("Muhammad Hamdan Bhatti")}`);
        const transactionInput = await inquirer.prompt([
            {
                type: "list",
                name: "accountType",
        ...
        if (userPin && transactionInput.transactionType === "Balance Inquiry") {
            ...
            console.log(`Your Current Balance is Rs ${chalk.red(userBalance)}\n${chalk.bgGray.yellow("Thanks For Using My ATM ....")}\n${chalk.green("This Code Written By Muhammad Hamdan Bhatti")} `);
        }
        else if (userPin) {
            const userBalance2 = 50000;
            if (userBalance2 >= enterAmount) {
                console.log(`Your account has been debited with ${chalk.red("RS" + enterAmount)} and Your Remaining balance is ${chalk.red("RS " + (userBalance2 - enterAmount))}\n\n${chalk.bgGray.yellow("Thanks For Using My ATM ....")} \n\n${chalk.green("This Code Written By Muhammad Hamdan Bhatti")} `);
            }
            else {
                console.log(`${chalk.red("Insufficient Balance")} \n ${chalk.bgGray.yellow("Thanks For Using ATM...  ")}\n ${chalk.green("This Code Written By Muhammad Hamdan Bhatti")} `);
            }
        }
        break;
    }
        ...
        if (loginAttempts < maxLoginAttempts) {
            console.log(`You entered the wrong PIN. You have ${maxLoginAttempts - loginAttempts} attempts left.`);
            console.log(`If you Enter ${maxLoginAttempts - loginAttempts} Wrong Pin, Your account will be locked\n`);
        }
        else {
            accountLocked = true;
            console.log(`Maximum login attempts reached.${chalk.red(" ==> Account locked")}.\n`);
        }
    }
}
if (accountLocked) {
    console.log("Your account has been locked. Contact the administrator\n");
    console.log(`${chalk.yellow("===========>")} ${chalk.bgGray.yellow("Thanks For Using ATM...  ")} ${chalk.yellow("<===========")} \n
  ${chalk.green("This Code Written By Muhammad Hamdan Bhatti")}`);
}
